...
gets the event directly. Otherwise set `PATHLOCKD_PEERS` to internal Raft
endpoints so each replica forwards events through the Raft-transport
`PublishEvent` RPC. Forwarding is done by one
